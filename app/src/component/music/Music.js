...
export default class Music extends Component {
  ...
  render() {
    ...
    //如果加载完毕重新刷新界面改变load值
    if (!this.state.load) {
      return <View style={{alignItems:'center', justifyContent:'center', width:width, height:height, backgroundColor:'#fff'}}>
          <Text>加载中...</Text>
      </View>
    }
    ...
    return (
      <View style={{flex:1, height:70}}>
        <View style={{height:69.5, flexDirection:'row'}}>
          <Image source={{uri:artist.portrait}} style={{width:50, height:50, margin:10, borderRadius:8}}/>
